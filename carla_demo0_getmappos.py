...
def game_loop(args):
    ...
    try:
        ...
        #################################
        ##在图上画点，获得地图上点对应的具体位置，并设置spectator观察者位置        
        
        #spawn_points=world.map.get_spawn_points()#1.随机waypoint
        


        # 2.定义X和Y的范围
        x_range = (254, 304)  # 示例范围，可以根据需要调整
        y_range = (-250, -172)  # 示例范围，可以根据需要调整
        z_fixed = 0.26  # 固定Z值
        ...
        # 生成100个随机点
        for _ in range(10):
            x = random.uniform(x_range[0], x_range[1])  # 在范围内随机选择X坐标
            y = random.uniform(y_range[0], y_range[1])  # 在范围内随机选择Y坐标
            location = carla.Location(x=x, y=y, z=z_fixed)
            locations.append(location)
            spawn_points = locations

        for i, transform in enumerate(spawn_points):
            #location = transform.location # 获取位置 (carla.Location)
            location = transform  # 直接使用transform作为位置
            coord_text = f"({location.x:.1f}, {location.y:.1f})"
            ...
            client.get_world().debug.draw_point(
                        location, 
                        size=0.1,  # 点的大小
                        color=carla.Color(r=255, g=255, b=0),  # 绿色标记
                        life_time=50,  # 显示时间（秒）
                        )
        ...
        location.z = 0.25
        angle = 0
        spectator.set_transform(get_transform(location, angle,height = 100,pitchAngle = 70))


        ###
        # 3.在地图停车场上生成一个随机点（Town4地图失败，所有路径规划基于waypoint，停车场区域没有waypoint）
        ...
        blueprint = client.get_world().get_blueprint_library().filter('model3')[0]
        #client.get_world().try_spawn_actor(blueprint, pos)
        posPark1 = pos    

        ################################
        ##设定目标点为停车位
        #agent.set_destination(agent.vehicle.get_location(), posPark1.location, clean=True)
...
def main():
    ...
    # --- 配置 ---
    """
        CARLA_ROOT = "D:\carla\carla-0-9-11-windows\WindowsNoEditor"  # <-- 必须修改为你的CARLA实际路径！
    MAP = "Town07"                       # 想要加载的地图
    PORT = 2000                          # 服务器端口

    # --- 构建命令 ---
    # 根据系统选择可执行文件 (Windows: .exe, Linux: .sh)
    exe = "CarlaUE4.exe" if os.name == 'nt' else "CarlaUE4.sh"
    cmd = [
    os.path.join(CARLA_ROOT, exe),
    f"-world-port={PORT}",
    f"-load D:\carla\carla-0-9-11-windows\WindowsNoEditor\HDMaps\{MAP}\{MAP}", # 加载指定地图
    "-carla-server",  # 启动为服务器模式
    "-nosound",       # 禁用声音
    "-carla-no-hud"   # 不显示HUD
    ]

    # --- 启动服务器 ---
    print(f"启动CARLA服务器 (地图: {MAP}, 端口: {PORT})...")
    #process = subprocess.Popen(cmd, cwd=CARLA_ROOT)
