...
import cv2
from useSAM  import     process_carla_imagesamseg  # pylint: disable=import-error      
...
def img_process(image):
    ...
    img_rgb, mask = process_carla_imagesamseg(array)  # 调用图像处理函数
    # 显示图像
    #cv2.imshow('CARLA RGB Sensor', array)
    #cv2.waitKey(100)
    if img_rgb is not None:
        str1 = 'out/outputannotated%06d.jpg' % image.frame
        cv2.imwrite(str1, img_rgb)

        str2= 'out/outputmask%06d.jpg' % image.frame
        cv2.imwrite(str2,mask)
